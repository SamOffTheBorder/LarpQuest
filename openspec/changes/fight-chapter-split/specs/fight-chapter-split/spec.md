...
When a turn in `action` mode has submissions from exactly two distinct entities acting adversarially against each other, and the turn is not itself a continuation, the narrator SHALL be instructed to end its streamed prose with an exact turning-point marker line if and only if it is signaling a turning point rather than a full resolution. When a turn's submissions do not meet this eligibility condition, or the turn is itself already a continuation (per `continues_chapter_id`), `action` mode's prompt MUST NOT mention the marker, and `generateTurn` MUST treat any occurrence of the marker in the resulting prose as absent — stripped without triggering a turning point — regardless of what the model emitted.

#### Scenario: Eligible 1v1 turn may signal a turning point
- **WHEN** a turn in `action` mode has submissions from exactly two distinct entities and the model judges the exchange has reached a dramatic turning point rather than a resolution
- **THEN** the narrator's streamed prose ends with the turning-point marker

#### Scenario: Group fight cannot signal a turning point
- **WHEN** a turn in `action` mode has submissions from three or more distinct entities

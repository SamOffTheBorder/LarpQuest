# universe-versioning Specification

## Purpose

Universes as versioned records independent of any story, owned by their creator, with immutable published `universe_versions` bundling an `entity_schema` and a `progression_model`. Stories pin an explicit universe version that persists unchanged except through an explicit owner-initiated upgrade.

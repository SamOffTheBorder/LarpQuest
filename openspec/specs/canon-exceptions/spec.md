# canon-exceptions Specification

## Purpose

GM/owner override action that writes a permanent, narrowly-scoped `canon_exceptions` row from either a validation flag or a Gatekeeper verdict, checked by the rule engine and the Gatekeeper before re-flagging the same combination.

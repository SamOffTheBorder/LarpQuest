# chapter-video Specification

## Purpose

Anime-style video generation seeded from a chapter's completed illustration: the `videographer` role, per-story opt-in (off by default), an asynchronous durable job, and access control.
...
### Requirement: Video generation requires deployment configuration
Requesting or retrying video generation SHALL fail with a clear, distinguishable error when the deployment has not configured a video-generation provider, rather than dispatching a job that can only fail later with an opaque provider error.

#### Scenario: Provider not configured
- **WHEN** an owner or GM requests video generation on a deployment with no video provider credentials configured
- **THEN** the system rejects the request immediately, without creating a job, with an error identifying the missing configuration
...
Generated chapter videos SHALL be readable only by members of the owning story, except when served through a share-link-derived signed URL (see the `share-links` capability — such a URL remains valid until its own short expiry even after the originating link is revoked).
...
- **WHEN** a user who is not a member of the story requests a chapter's generated video directly (not via a share link)

...
Pure evaluation of a chapter draft against the Standard Rule Pack and a universe's research-derived `validation_rules`, filtered by `applies_when`/`progression_models`, producing severity-tagged flags. Also owns `canon_exceptions` suppression lookup.

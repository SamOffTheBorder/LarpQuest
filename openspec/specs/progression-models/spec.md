# progression-models Specification

## Purpose

A dispatch-table-based resolution of universe progression semantics, mirroring the turn-mode dispatch pattern. Registers the `none` and `ability_unlock` progression models, and requires that the turn loop, entity write path, and context assembly run structurally incompatible universes without branching on universe, genre, or media type.

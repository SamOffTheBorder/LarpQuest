...
Per-chapter structured summary and embedding generation on publish (non-blocking, mirroring extraction), and long-story arc-summary compaction beyond the arc-compaction threshold, so vector retrieval has material to draw on and context growth does not stay linear in chapter count.
